...
public class VigenereCipher {
    ...
    /**
     * Determines the keyword length between the signature test and the kasiski
     * test.
     * 
    ...
    private static int determineKeywordLength(int signatureTestGuess, Set<Integer> kasiskiGuess) {
        ...
        // find smallest distance between each Kasiski guess and Signature Test
        // guess
    ...
    private static String determineKey(List<Character> cipherText, int keyLength) {
        ...
        for (int i = 0; i < cosets.size(); i++) {
            int shift = ShiftCipher.autoDecrypt(CipherUtil.convertCharacterListToString(cosets.get(i)),
                    new CipherLogger()) % CipherUtil.NUM_OF_CHARS;
