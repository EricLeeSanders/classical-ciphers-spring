...
<body>
    ...
    <div id="main-content">
      <div id="cipher-input">
      <sf:form method="post" action="${pageContext.request.contextPath}/shift" commandName="shift">
        <div id ="shift-cipher-input" class="selected-cipher">
          <label> Shift Amount: </label>
          <select name="shiftAmount" id="shift-amount">
		    <c:forEach var="validShift" items="${shift.validShifts}">
				<option value="${validShift}" ${validShift == shift.shiftAmount ? 'selected' : ''}>${validShift}</option>
		    </c:forEach>
          </select>
        </div>
        <br>
        ...
        <br>
        <textarea name="plainText" id="plain-text" placeholder="Plain text"><c:out value="${shift.plainText}"></c:out></textarea>
        <br>
        <label>Cipher Text</label>
        <br>
        <textarea name="cipherText" id="cipher-text" placeholder="Cipher text"><c:out value="${shift.cipherText}"></c:out></textarea>
        <br>
