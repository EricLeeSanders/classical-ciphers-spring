...
<body>
    ...
    <div id="main-content">
      <div id="cipher-input">
      <sf:form method="post" action="${pageContext.request.contextPath}/affine" commandName="affine">
        <div id ="affine-cipher-input" class="selected-cipher">
          <label> Shift Amount A: </label>
          <select name="shiftAmountA" id="shift-amount-A">
		    <c:forEach var="validShift" items="${affine.validShiftsA}">
				<option value="${validShift}" ${validShift == affine.shiftAmountA ? 'selected' : ''}>${validShift}</option>
		    </c:forEach>
          </select>
          <br>
          <br>
          <label> Shift Amount B: </label>
          <select name="shiftAmountB" id="shift-amount-B">
		    <c:forEach var="validShift" items="${affine.validShiftsB}">
				<option value="${validShift}" ${validShift == affine.shiftAmountB ? 'selected' : ''}>${validShift}</option>
		    </c:forEach>
          </select>
        </div>
        <br>
        ...
        <br>
        <textarea name="plainText" id="plain-text" placeholder="Plain text"><c:out value="${affine.plainText}"></c:out></textarea>
        <br>
        <label>Cipher Text</label>
        <br>
        <textarea name="cipherText" id="cipher-text" placeholder="Cipher text"><c:out value="${affine.cipherText}"></c:out></textarea>
        <br>
     ...
     <form action="<c:url value="/randomaffine" />" method="GET">
