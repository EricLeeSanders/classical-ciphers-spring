		...
		.ready(
				function() {
					...
					function buildCipherPostURL() {
						return cipherType.toLowerCase() + "/"
								+ cipherDirection.toLowerCase();
					}

					function buildCipherExampleURL() {
						return ctx + "/assets/cipherexamples/"
								+ cipherType.toLowerCase() + "-examples.json";
					}
					...
					$('#cipher-example')
							.click(
									function(e) {
										$
												.getJSON(
														buildCipherExampleURL(),
														function(data) {
															// Get a random
															// example
															var rndIdx = getRandomInt(
																	0,
																	Object
																			.keys(data).length);
															// Only add the key
															...
															if (cipherDirection === "Encrypt") {
																$('#plain-text')
																		.val(
																				data[rndIdx].plainText);
															} else {
																$(
																		'#cipher-text')
																		.val(
																				data[rndIdx].cipherText);
															}
														});
					...
					$('#cipher-type-buttons > button')
							.click(
									function(e) {
										...
										// remove 'active' class from all key
										// panels
										$('.active-key-panel').removeClass(
												'active-key-panel');
										// add 'active' class to corresponding
										// key panel
										$(
												'#' + cipherType.toLowerCase()
														+ '-key-panel')
					...
					$('#cipher-direction-buttons > button').click(
							function(e) {
								e.preventDefault();
								$(this).siblings('button.active').removeClass(
										'active');
								$(this).addClass('active');
					...
					function disableControls() {
						...
						$('.active-key-panel .form-control').prop('disabled',
								inputDisabled);
					}
